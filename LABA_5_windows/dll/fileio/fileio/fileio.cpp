...
extern "C" __declspec(dllexport) void read() { //что функция read будет видна вне DLL
	//создаем события
	event_output = CreateEvent(nullptr, false, false, nullptr);
	event_input = CreateEvent(nullptr, false, true, nullptr);
	event_close = CreateEvent(nullptr, false, false, nullptr);
...
extern "C" __declspec(dllexport) void write() { //что функция write будет видна вне DLL
	...
	while (true) {
		...
		if (WaitForSingleObject(event_close, 0) == WAIT_OBJECT_0) { // ожидаем засигнала о завершении
			break;
		}
	}
